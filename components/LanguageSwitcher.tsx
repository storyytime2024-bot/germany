...
function getCurrentLanguage(): string {
  const match = document.cookie.match(/googtrans=\/de\/(\w+)/);
    ...
    return (
      <div ref={ref} className="fixed bottom-6 right-6 z-[999]">
        <button
          ...
          className={`group flex items-center gap-3 px-5 py-4 rounded-2xl bg-gradient-to-br from-red-600 to-amber-500 shadow-2xl shadow-red-500/30 hover:shadow-red-500/50 hover:scale-105 active:scale-95 transition-all duration-300 border border-white/10 ${open ? 'scale-105' : ''}`}
        ...
        {open && (
          <div className="absolute bottom-full right-0 mb-3 w-64 bg-slate-900/95 backdrop-blur-xl border border-white/10 rounded-2xl shadow-2xl shadow-black/60 overflow-hidden max-h-[420px] overflow-y-auto custom-scrollbar">
            <div className="px-5 py-3 border-b border-white/5 flex items-center gap-2">
              ...
              <span className="text-sm font-bold text-white/70 uppercase tracking-wider">Übersetzen</span>
            </div>
            {languages.map(lang => (
              <button
                key={lang.code}
              ...
              >
                ...
                {lang.code === selected.code && (
                  <svg className="w-5 h-5 text-red-400 ml-auto" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24">
            ...
            {languages.map(lang => (
              <button
                ...
                className={`flex items-center gap-3 w-full px-5 py-3 text-left text-base font-semibold transition-colors ${
                  lang.code === selected.code ? 'bg-red-600/20 text-white' : 'text-slate-300 hover:bg-white/5 hover:text-white'
                }`}
              >
          ...
          {languages.map(lang => (
            <button
              ...
              className={`flex items-center gap-3 w-full px-4 py-2.5 text-left text-sm font-medium transition-colors ${
                lang.code === selected.code ? 'bg-red-600/20 text-white' : 'text-slate-400 hover:bg-white/5 hover:text-white'
              }`}
            >
              <span>{lang.flag}</span>
              <span>{lang.label}</span>
