...
const Testimonials: React.FC = () => {
  return (
    <div className="container mx-auto px-6 text-center">
      <span className="px-6 py-2 rounded-full border border-white/20 bg-white/5 text-xs font-bold tracking-widest uppercase mb-6 inline-block">23000+ ZUFRIEDENE KUNDEN</span>
      <h2 className="text-4xl md:text-6xl font-black mb-16">#1 IPTV in Deutschland laut unseren Kunden</h2>

      <div className="max-w-3xl mx-auto space-y-8">
        {TESTIMONIALS.map((t, i) => (
          <div key={i} className={`flex items-start gap-4 ${t.side === 'right' ? 'flex-row-reverse' : ''}`}>
            <div className={`w-10 h-10 rounded-full flex items-center justify-center font-bold text-xs shrink-0 ${t.side === 'left' ? 'bg-blue-600' : 'bg-amber-600'}`}>
              {t.avatar}
            </div>
      ...
      <div className="mt-20">
        <span className="text-xs font-bold text-gray-500 uppercase tracking-widest mb-8 block">ZAHLUNGSMETHODEN</span>
        <h3 className="text-3xl font-black mb-10">Zahle wie du willst</h3>
        <div className="flex flex-wrap justify-center gap-4 md:gap-8">
          {[
            { name: 'SEPA', icon: <svg className="w-8 h-8" viewBox="0 0 48 48" fill="none"><rect width="48" height="48" rx="8" fill="#003399"/><text x="24" y="28" textAnchor="middle" fill="white" fontSize="11" fontWeight="bold" fontFamily="sans-serif">SEPA</text></svg> },
            ...
            { name: 'Bank', icon: <svg className="w-8 h-8" viewBox="0 0 24 24" fill="none" stroke="#f87171" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M3 21h18M3 10h18M5 6l7-3 7 3M4 10v11M20 10v11M8 14v4M12 14v4M16 14v4"/></svg> },
